...
public class Spawner : MonoBehaviour {
	...
	IEnumerator SpawnEnemy() {
		...
		if (isCamping) {
            Transform tile = map.GetTileFromPosition(playerT.position);
            if (tile != null)
            {
                spawnTile = tile;
            }
		...
		Color initialColour = Color.gray;
        ...
        while (spawnTimer < spawnDelay)
        {
            ...
            tileMat.color = Color.LerpUnclamped(initialColour, flashColour, Mathf.PingPong(spawnTimer * tileFlashSpeed, 1));

            spawnTimer += Time.deltaTime;
            yield return null;
        }
        tileMat.color = Color.clear;
